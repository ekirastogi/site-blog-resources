...
{var flashcopier=sender.parentNode.parentNode.parentNode.appendChild(document.createElement('div'));
...
{var wnd=window.open('','_blank','dialog,width=300,height=200,scrollbars=0');var doc=wnd.document;dp.sh.Utils.CopyStyles(doc,window.document);doc.write(dp.sh.Strings.AboutDialog.replace('{V}',dp.sh.Version));doc.close();wnd.focus();}}};dp.sh.Toolbar.Create=function(highlighter)
...
this.AddBit(this.code.substr(pos),null);this.SwitchToList();this.div.appendChild(this.bar);this.div.appendChild(this.ol);setTimeout("var lists = document.getElementsByTagName('ol'); for(var j = 0; j < lists.length; j++){ if(lists[j].parentNode.className == \"dp-highlighter\"){ var maxWidth = lists[j].offsetWidth; var originalWidth = lists[j].offsetWidth; for(var i = 0; i < lists[j].childNodes.length; i++){ if(maxWidth < lists[j].childNodes[i].firstChild.offsetWidth){ maxWidth = lists[j].childNodes[i].firstChild.offsetWidth; } } if(maxWidth > originalWidth){lists[j].previousSibling.style.width = '' + (maxWidth + 14) + 'px';lists[j].style.width = '' + (maxWidth + 14) + 'px'; } } }", 1);}
